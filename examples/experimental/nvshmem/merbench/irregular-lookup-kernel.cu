...
template <typename T>
__global__ void kernel(size_t num_threads, size_t transfer_size, size_t num_lookups,
  ...
  for (size_t i = 0; i < num_lookups; i++) {
    ...
    */
    size_t dest_rank = rand_nums[ltid + i*local_extent*2] % array.d_ptrs_.size();
    /*
    ...
    */
    size_t rand_loc = rand_nums[ltid+local_extent + i*local_extent*2] % (array.local_size() - transfer_size);
    /*
    ...
    */
    T* buf = buffer.data() + ltid * transfer_size;
...
void run_kernel(size_t transfer_data_size) {
  ...
  kernel<<<num_blocks, threads_per_block>>>
           (num_threads, transfer_size, num_lookups, rand_nums, buffer, array);
