...
void async_gemm(const BCL::SPMatrix<T, index_type>& a,
  ...
  for (size_t i = 0; i < c.grid_shape()[0]; i++) {
    for (size_t j = 0; j < c.grid_shape()[1]; j++) {
      if (c.tile_locale(i, j) == BCL::rank()) {
        ...
        *claimants[i*c.grid_shape()[1] + j] = -1;
      }
    }
  }
  ...
  for (size_t i = 0; i < c.grid_shape()[0]; i++) {
    for (size_t j = 0; j < c.grid_shape()[1]; j++) {
      claimants[i*c.grid_shape()[1] + j] = BCL::broadcast(claimants[i*c.grid_shape()[1] + j], c.tile_locale(i, j));
    }
  }
  ...
  for (size_t i = 0; i < c.grid_shape()[0]; i++) {
    for (size_t j = 0; j < c.grid_shape()[1]; j++) {
      if (c.tile_locale(i, j) == BCL::rank()) {
        ...
        int result = BCL::compare_and_swap(claimants[i*c.grid_shape()[1] + j], -1, (int) BCL::rank());
        // If result is uninitialized => you've claimed it.
        if (result == -1) {
  ...
  for (size_t i = 0; i < c.grid_shape()[0]; i++) {
    for (size_t j = 0; j < c.grid_shape()[1]; j++) {
      ...
      // If result is uninitialized => you've claimed it.
      if (result == -1) {
        printf("%lu has stolen tile (%lu, %lu) from %lu\n",
               BCL::rank(), i, j, c.tile_locale(i, j));
        size_t k_offset = j % a.grid_shape()[1];
  ...
  load += duration;
  begin = std::chrono::high_resolution_clock::now();
  std::vector<size_t> locales(claimants.size());
  for (size_t i = 0; i < locales.size(); i++) {
    size_t i_ = (i + BCL::rank()) % locales.size();
    locales[i_] = *claimants[i_];
  }
  c.rebroadcast_tiles(locales);
  // c.rebroadcast_tiles();
  end = std::chrono::high_resolution_clock::now();
  duration = std::chrono::duration<double>(end - begin).count();
