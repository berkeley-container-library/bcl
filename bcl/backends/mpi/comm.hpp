...
inline void read(const GlobalPtr <T> &src, T *dst, const size_t size) {
  ...
  */
  BCL_DEBUG( int error_code; )
  MPI_Request request;

  BCL_DEBUG( error_code = )
  MPI_Rget(dst, size*sizeof(T), MPI_CHAR, src.rank, src.ptr, size*sizeof(T),
    MPI_CHAR, BCL::win, &request);
  BCL_DEBUG(
    if (error_code != MPI_SUCCESS) {
      throw debug_error("BCL read(): MPI_Rget return error code " + std::to_string(error_code));
    }
  )
  ...
  BCL_DEBUG(
    if (error_code != MPI_SUCCESS) {
      throw debug_error("BCL read(): MPI_Wait return error code " + std::to_string(error_code));
    }
  )
}
...
inline BCL::request async_write(const T* src, const GlobalPtr<T>& dst, size_t size) {
  ...
  BCL_DEBUG( error_code = )
  MPI_Rput(src, size*sizeof(T), MPI_CHAR, dst.rank, dst.ptr,
           size*sizeof(T), MPI_CHAR, BCL::win, &request);
  BCL_DEBUG(
          if (error_code != MPI_SUCCESS) {
...
inline void broadcast(T* val, uint64_t root, size_t size, const BCL::Team& team) {
  ...
  if (team.in_team()) {
    BCL_DEBUG( int error_code; )
    BCL_DEBUG( error_code = )
    MPI_Bcast(val, size*sizeof(T), MPI_CHAR, root, comm.comm());
    BCL_DEBUG(
            if (error_code != MPI_SUCCESS) {
              throw debug_error("BCL broadcast(): MPI_Bcast return error code " + std::to_string(error_code));
            }
...
inline auto arbroadcast(T* val, uint64_t root, size_t size, const BCL::Team& team) {
  ...
  if (team.in_team()) {
    BCL_DEBUG( int error_code; )
    BCL_DEBUG( error_code = )
    MPI_Ibcast(val, size*sizeof(T), MPI_CHAR, root, comm.comm(), &request);
    BCL_DEBUG(
            if (error_code != MPI_SUCCESS) {
...
inline void broadcast(std::vector<T, Allocator>& val, uint64_t root, const BCL::Team& team) {
  ...
  if (team.in_team()) {
    BCL_DEBUG( int error_code; )
    BCL_DEBUG( error_code = )
    MPI_Bcast(val.data(), val.size()*sizeof(T), MPI_CHAR, root, comm.comm());
    BCL_DEBUG(
            if (error_code != MPI_SUCCESS) {
              throw debug_error("BCL broadcast(): MPI_Bcast return error code " + std::to_string(error_code));
            }
...
inline T broadcast(T& val, uint64_t root, const BCL::Team& team) {
  ...
  if (team.in_team()) {
    BCL_DEBUG( int error_code; )
    BCL_DEBUG( error_code = )
    MPI_Bcast(&val, sizeof(T), MPI_CHAR, root, comm.comm());
    BCL_DEBUG(
            if (error_code != MPI_SUCCESS) {
              throw debug_error("BCL broadcast(): MPI_Bcast return error code " + std::to_string(error_code));
            }
...
inline BCL::future<T> arfetch_and_op(const GlobalPtr<T> ptr, const T& val, const atomic_op<T>& op) {
  ...
  BCL_DEBUG( error_code = )
  MPI_Rget_accumulate(&val, 1, op.type(), future.value_.get(), 1, op.type(), ptr.rank, ptr.ptr,
    1, op.type(), op.op(), BCL::win, &request);
  BCL_DEBUG(
          if (error_code != MPI_SUCCESS) {
